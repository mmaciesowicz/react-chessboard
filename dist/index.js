 ...
 */
function getRelativeCoords(boardDimensions = { rows: 8, columns: 8 }, boardOrientation, boardWidth, square) {
 ...
 */
function convertPositionToObject(position, boardDimensions) {
    if (position === "start") {
 ...
 */
function fenToObj(fen, boardDimensions = { rows: 8, columns: 8 }) {
    if (!isValidFen(fen, boardDimensions))
        return {};
        ...
        for (let j = 0; j < row.length; j++) {
            ...
            if (numericRegex.test(row[j])) {
                ...
                if (numEmptySquares >= 1 && numEmptySquares <= boardDimensions.columns) {
                    ...
                    throw new Error(`Invalid FEN: empty square count (${numEmptySquares}) exceeds board dimensions (${boardDimensions.columns})`);
                }
            }
            else {
                // piece
                const dynamicColumns = Array.from({ length: boardDimensions.columns }, (_, i) => String.fromCharCode(97 + i) // 97 is 'a'
                );
 ...
 */
function isValidFen(fen, boardDimensions = { rows: 8, columns: 8 }) {
    // cut off any move, castling, etc info from the end. we're only interested in position information
    fen = fen.replace(/ .+$/, "");
    ...
    if (chunks.length !== boardDimensions.rows)
        return false;
    // check each section
    for (let i = 0; i < boardDimensions.rows; i++) {
...
const ChessboardProvider = react.forwardRef(({ allowDragOutsideBoard = true, animationDuration = 300, areArrowsAllowed = true, arePiecesDraggable = true, arePremovesAllowed = false, autoPromoteToQueen = false, boardDimensions = { rows: 8, columns: 8 }, boardOrientation = "white", boardWidth, children, clearPremovesOnRightClick = true, customArrows, customArrowColor = "rgb(255,170,0)", customBoardStyle, customNotationStyle, customDarkSquareStyle = { backgroundColor: "#B58863" }, customDropSquareStyle = {
    ...
    const [currentPosition, setCurrentPosition] = react.useState(convertPositionToObject(position, boardDimensions));
...
function Notation({ row, col }) {
    const { boardDimensions, boardOrientation, boardWidth, customDarkSquareStyle, customLightSquareStyle, customNotationStyle, } = useChessboard();
    const dynamicColumns = Array.from({ length: boardDimensions.columns }, (_, i) => String.fromCharCode(97 + i) // 97 is 'a'
    );
    const squareWidth = boardWidth / boardDimensions.columns;
    ...
    function renderBottomLeft() {
        return (jsxRuntime.jsxs(jsxRuntime.Fragment, { children: [jsxRuntime.jsx("div", Object.assign({ style: Object.assign(Object.assign({ userSelect: "none", zIndex: 3, position: "absolute" }, { color: ((boardOrientation === "white") ? whiteColor : ((boardDimensions.rows % 2 === 0) === (boardDimensions.columns % 2 === 0)) ? whiteColor : blackColor) }), numericStyle(squareWidth, customNotationStyle)) }, { children: getRow() })), jsxRuntime.jsx("div", Object.assign({ style: Object.assign(Object.assign({ userSelect: "none", zIndex: 3, position: "absolute" }, { color: ((boardOrientation === "white") ? whiteColor : ((boardDimensions.rows % 2 === 0) === (boardDimensions.columns % 2 === 0)) ? whiteColor : blackColor) }), alphaStyle(squareWidth, customNotationStyle)) }, { children: getColumn() }))] }));
    }
    function renderLetters() {
        return (jsxRuntime.jsx("div", Object.assign({ style: Object.assign(Object.assign({ userSelect: "none", zIndex: 3, position: "absolute" }, { color: boardOrientation === "white" ? ((col % 2 === 0) ? whiteColor : blackColor) : ((col % 2 !== 0) === (boardDimensions.rows % 2 === 0) === (boardDimensions.columns % 2 === 0) ? blackColor : whiteColor) }), alphaStyle(squareWidth, customNotationStyle)) }, { children: getColumn() })));
    }
    function renderNumbers() {
        return (jsxRuntime.jsx("div", Object.assign({ style: Object.assign(Object.assign({ userSelect: "none", zIndex: 3, position: "absolute" }, ({ color: boardOrientation === "white" ? ((row % 2 === 0) === (boardDimensions.rows % 2 !== 0) ? whiteColor : blackColor) : ((row % 2 === 0) === (boardDimensions.columns % 2 !== 0) ? whiteColor : blackColor) })), numericStyle(squareWidth, customNotationStyle)) }, { children: getRow() })));
    }
...
const alphaStyle = (width, customNotationStyle) => (Object.assign({ right: width / 48, bottom: 0, fontSize: width / 6.2 }, customNotationStyle));
const numericStyle = (width, customNotationStyle) => (Object.assign({ top: 0, left: width / 48, fontSize: width / 6.2 }, customNotationStyle));
...
function Piece({ isPremovedPiece = false, piece, square, squares, }) {
    const { animationDuration, arePiecesDraggable, boardDimensions, boardWidth, boardOrientation, chessPieces, currentPosition, deletePieceFromSquare, dropOffBoardAction, id, isDraggablePiece, isWaitingForAnimation, onPieceClick, onPieceDragBegin, onPieceDragEnd, onPieceDropOffBoard, onPromotionCheck, positionDifferences, } = useChessboard();
    const squareWidth = boardWidth / boardDimensions.columns;
...
function Square({ square, squareColor, setSquares, squareHasPremove, children, }) {
    ...
    const defaultSquareStyle = Object.assign(Object.assign(Object.assign(Object.assign({}, borderRadius(square, boardDimensions, boardOrientation, customBoardStyle)), (squareColor === "black"
        ? customDarkSquareStyle
    ...
    return (jsxRuntime.jsx("div", Object.assign({ ref: drop, style: defaultSquareStyle, "data-square-color": squareColor, "data-square": square, onTouchMove: (e) => {
        ...
        , Object.assign({ 
            // Type is too complex to properly evaluate, so ignore this line.
            // @ts-ignore
            ref: squareRef, style: Object.assign(Object.assign(Object.assign(Object.assign({}, size(boardWidth, boardHeight, boardDimensions)), center), (!squareHasPremove && (customSquareStyles === null || customSquareStyles === void 0 ? void 0 : customSquareStyles[square]))), { position: 'relative' }) }, { children: children }))) : (jsxRuntime.jsx(CustomSquare, Object.assign({ ref: squareRef, square: square, squareColor: squareColor, style: Object.assign(Object.assign(Object.assign({}, size(boardWidth, boardHeight, boardDimensions)), center), (!squareHasPremove && (customSquareStyles === null || customSquareStyles === void 0 ? void 0 : customSquareStyles[square]))) }, { children: children }))) })));
}
const center = {
    display: "flex",
    position: "relative",
};
const size = (width, height, boardDimensions = { rows: 8, columns: 8 }) => ({
    width: width / boardDimensions.columns,
    height: height / boardDimensions.rows,
});
const borderRadius = (square, boardDimensions = { rows: 8, columns: 8 }, boardOrientation, customBoardStyle) => {
...
function Squares() {
    ...
    const { arePremovesAllowed, boardDimensions, boardOrientation, boardWidth, currentPosition, id, premoves, showBoardNotation, } = useChessboard();
    const dynamicColumns = Array.from({ length: boardDimensions.columns }, (_, i) => String.fromCharCode(97 + i) // 97 is 'a'
    );
    ...
    return (jsxRuntime.jsx("div", Object.assign({ "data-boardid": id }, { children: [...Array(boardDimensions.rows)].map((_, r) => {
            return (jsxRuntime.jsx("div", Object.assign({ style: {
                    display: "flex",
                    flexWrap: "nowrap",
                    width: boardWidth,
                } }, { children: [...Array(boardDimensions.columns)].map((_, c) => {
                    const square = boardOrientation === "black"
                        ? (dynamicColumns[boardDimensions.columns - 1 - c] + (r + 1))
                        : (dynamicColumns[c] + (boardDimensions.rows - r));
                    const squareColor = (r + c) % 2 === 0 === (boardOrientation === "white" ? boardDimensions.rows % 2 !== 0 : boardDimensions.columns % 2 !== 0) ? "black" : "white";
                    const squareHasPremove = premoves.some((p) => p.sourceSq === square || p.targetSq === square);
                    const squareHasPremoveTarget = premovesHistory
                        ...
                        .sort((a, b) => { var _a, _b; return ((_a = b.premovesRoute.at(-1)) === null || _a === void 0 ? void 0 : _a.index) - ((_b = a.premovesRoute.at(-1)) === null || _b === void 0 ? void 0 : _b.index); })
                        .at(0);
...
const Arrows = () => {
    const { arrows, newArrow, boardDimensions, boardOrientation, boardWidth, customArrowColor: primaryArrowCollor, } = useChessboard();
    const boardHeight = (boardWidth * boardDimensions.rows) / boardDimensions.columns;
    const squareWidth = boardWidth / boardDimensions.columns;
    const arrowsList = [...arrows, newArrow].filter(Boolean);
    return (jsxRuntime.jsx("svg", Object.assign({ width: boardWidth, height: boardHeight, style: {
            position: "absolute",
            top: "0",
            left: "0",
            pointerEvents: "none",
            ...
            if (arrowStartField === arrowEndField)
                return null;
            const from = getRelativeCoords(boardDimensions, boardOrientation, boardWidth, arrowStartField);
            const to = getRelativeCoords(boardDimensions, boardOrientation, boardWidth, arrowEndField);
            ...
            return (jsxRuntime.jsxs(react.Fragment, { children: [jsxRuntime.jsx("marker", Object.assign({ id: `arrowhead-${i}`, markerWidth: "2", markerHeight: "2.5", refX: "1.25", refY: "1.25", orient: "auto" }, { children: jsxRuntime.jsx("polygon", { points: "0.3 0, 2 1.25, 0.3 2.5", fill: arrowColor !== null && arrowColor !== void 0 ? arrowColor : primaryArrowCollor }) })), jsxRuntime.jsx("line", { x1: from.x, y1: from.y, x2: end.x, y2: end.y, opacity: isArrowActive ? "0.5" : "0.65", stroke: arrowColor !== null && arrowColor !== void 0 ? arrowColor : primaryArrowCollor, strokeWidth: isArrowActive ? 0.9 * squareWidth / 5.5 : squareWidth / 5.5, markerEnd: `url(#arrowhead-${i})` })] }, `${arrowStartField}-${arrowEndField}${isArrowActive ? "-active" : ""}`));
        }) })));
};

function PromotionOption({ option }) {
    const [isHover, setIsHover] = react.useState(false);
    const { boardDimensions, boardWidth, chessPieces, customDarkSquareStyle, customLightSquareStyle, handleSetPosition, onPromotionPieceSelect, promoteFromSquare, promoteToSquare, promotionDialogVariant, } = useChessboard();
    ...
    return (jsxRuntime.jsx("div", Object.assign({ onClick: () => {
            ...
            transition: "all 0.1s ease-out",
                ...
                isDragging: false,
            }) }))) : (jsxRuntime.jsx("svg", Object.assign({ viewBox: "1 1 43 43", width: boardWidth / boardDimensions.columns, height: boardWidth / boardDimensions.columns, style: {
                transition: "all 0.1s ease-out",
                transform: isHover ? "scale(1)" : "scale(0.85)",
...
function PromotionDialog() {
    const { boardDimensions, boardOrientation, boardWidth, promotionDialogVariant, promoteToSquare, } = useChessboard();
    const promotePieceColor = (promoteToSquare === null || promoteToSquare === void 0 ? void 0 : promoteToSquare.slice(1, 3)) === "1" ? "b" : "w";
    ...
    const dialogCoords = getRelativeCoords(boardDimensions, boardOrientation, boardWidth, promoteToSquare || `a${boardDimensions.rows}`);
...
function Board() {
    ...
    const { boardWidth, boardDimensions, clearCurrentRightClickDown, onPromotionPieceSelect, setShowPromoteDialog, showPromoteDialog, customBoardStyle, } = useChessboard();
    const boardHeight = (boardWidth * boardDimensions.rows) / boardDimensions.columns;
    react.useEffect(() => {
...
const boardStyles = (width, height) => ({
    cursor: "default",
    height,
    width,
});
...
function CustomDragLayer({ boardContainer }) {
    const { boardWidth, boardDimensions, chessPieces, id, snapToCursor, allowDragOutsideBoard, } = useChessboard();
    const boardHeight = (boardWidth * boardDimensions.rows) / boardDimensions.columns;
    const squareWidth = boardWidth / boardDimensions.columns;
    ...
    return isDragging && item.id === id ? (jsxRuntime.jsx("div", Object.assign({ style: {
            ...
            top: 0,
                ...
                squareWidth,
                isDragging: true,
            })) : (jsxRuntime.jsx("svg", Object.assign({ viewBox: "1 1 43 43", width: squareWidth, height: squareWidth }, { children: jsxRuntime.jsx("g", { children: chessPieces[item.piece] }) }))) })) }))) : null;
}
